...
6. Action proposals → user approval → tool execution (gated)
...
- ✅ **Action Proposal & Gating**:
  - Agents propose actions (write_file, apply_patch, run_command)
  - ActionProposal schema with dry-run flag
  - User approval before execution
  - Tool executor with allowlist (safety tags)
- ✅ **Tracing**:
...
- ❌ No autonomous execution (all actions gated)
...
- [ ] Actions are proposed and gated; user approves before execution
...
- [ ] All write operations are gated; no unapproved modifications
...
propose_plan = ToolDefinition(
    ...
    requires_approval=False,  # output only
)
```
...
2. Tool executor checks `requires_approval=True` → wait for user approval
3. Dry-run mode (parse tool call, show diff/effect, ask approval)
4. Command runner has timeout + output limit
5. File operations respect allowed paths (configurable in config)

```python
class ToolExecutor:
    def __init__(self, agent_allowlist_tags: List[str], config: Config):
        self.agent_allowlist_tags = agent_allowlist_tags
        self.config = config  # allowed_paths, etc.
    ...
    async def execute_tool(
        ...
        # 3. If dry_run: return simulated effect
        # 4. Else: execute + return result
        ...
        for node_id_other, similarity, metadata in similar_nodes:
            if similarity > 0.7 and matches_heuristic(metadata):
                proposal = GraphUpdateProposal(
                    ...
                    confidence=0.85,
                    requires_user_approval=False,  # auto-low-confidence ok
                )
...
class ActionProposal(BaseModel):
    ...
    dry_run_output: Optional[str] = None  # what would happen
    justification: str
    requires_approval: bool = True
    created_at: datetime
    approved_at: Optional[datetime]
...
def task(
    ...
    async def _run():
        ...
        if result.proposed_actions:
            typer.echo("\n⚠️  Proposed Actions (awaiting approval):")
            for action in result.proposed_actions:
                typer.echo(f"   [{action.action_id}] {action.action_type}: {action.path}")
...
✅ Graph store + proposal system (with approval gating)  
✅ Mask system (heuristic weights)  
✅ Context Manager (graph-aware retrieval)  
✅ Multi-agent orchestration (5 agents)  
✅ Provider routing interface (OpenAI + mock)  
✅ Tool executor (allowlist + approval)  
✅ JSONL tracing  
✅ Feedback collection  
✅ CLI (8+ commands)  
...
4. **Gated automation** → user stays in control
